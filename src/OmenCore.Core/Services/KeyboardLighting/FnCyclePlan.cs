...
namespace OmenCore.Services.KeyboardLighting
{
    ...
    public static class FnCyclePlan
    {
        ...
        /// <summary>
        /// Black for anything unparseable, matching the hex boxes elsewhere in the UI.
        ///
        /// Hand-parsed rather than routed through WPF's <c>ColorConverter</c> - this type lives in
        /// OmenCore.Core, which deliberately has no WPF reference (see the project file), so it
        /// cannot use the same converter the WPF swatches do. Accepts "RRGGBB" and "AARRGGBB" (alpha
        /// read and discarded, since every wire colour here is opaque); anything else is black.
        /// </summary>
