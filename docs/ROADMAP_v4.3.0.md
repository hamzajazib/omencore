...
### Self-Caught Mistake: `8E10`'s Database Entry Had a Wrong `MaxFanLevel` That Would Have Introduced a Known Reassert-Loop Bug

**What happened.** Earlier this cycle, [#130](https://github.com/theantipopau/omencore/issues/130) (HP OMEN 17-db1180ng, RTX 5070) was used to add board `8E10` to `ModelCapabilityDatabase.cs`, cloning the conservative-but-still-optimistic profile shape of the CPU-generation-sibling `16-ap0xxx` boards (`8D24`/`8D26`/`8E35`) — including `MaxFanLevel = 55` and `HasMuxSwitch`/`SupportsGpuPowerBoost = true`. This was done **without checking for other open issues about the same ProductId first.**

**What was missed.** [#171](https://github.com/theantipopau/omencore/issues/171) — a separate, earlier, much more carefully-triaged report on the *same* board (a different reporter, RTX 5060 variant) — already had real Guided Fan Verification data and an explicit maintainer decision on record: *"we're not adding an 8E10 database entry yet... before promoting a board to a real profile we like to have explicit confirmation of things like RGB control actually working and a sustained... Max-hold test."* That thread's own data showed this board's **real Max-hold fan level settles at 45, not the nominal 55** ("`expected level 55, got level 45`" in the verification log) — and separately traced *exactly* this real-max-below-nominal-max pattern to a genuine, still-open bug: `WmiFanController`'s Max-mode floor check computes its threshold as `MaxFanLevel * MaxModeAbsoluteLevelFloorRatio` (0.50), and when the real ceiling sits below that derived floor, the check becomes permanently unwinnable, producing an endless "external reset suspected" reassert loop — already documented (same #171 thread) as a likely contributor to real in-game stutter on a related board via a 20-second reassert firing 14+ times in a 5-minute window.

**In other words:** shipping `8E10` with `MaxFanLevel = 55` would not just have been a wrong number — it would have actively introduced, on a board already flagged as having this exact real-ceiling-below-nominal shape, the specific unresolved bug pattern #171's own investigation had already identified and was deliberately avoiding by not promoting this board yet.

**How it was caught.** Reviewing GitHub for other open issues before continuing work surfaced #171 independently; cross-referencing it against the just-added `8E10` entry (rather than assuming the earlier addition was already correct) surfaced the conflict. Not caught by the test suite this time — unlike the `88D2` duplicate-entry near-miss earlier this cycle, a wrong-but-internally-consistent value doesn't fail a build or an existing assertion, only a broader piece of latent hardware knowledge check would.

**Fix.** Corrected the `8E10` entry: `MaxFanLevel = 45` (the field-confirmed real ceiling from #171, not the nominal 55), and `HasMuxSwitch`/`SupportsGpuPowerBoost` flipped to conservative `false` — neither is confirmed by either report, and inheriting them from the `16-ap0xxx` siblings purely because they share a CPU generation is exactly the family-template-inheritance mistake the `DefaultCapabilities`/`GetCapabilitiesByFamily` fallback fix earlier this cycle (see "Model Capability Fallbacks Were Optimistic" below) already corrected once at the fallback-path level — this was the same mistake recurring at the named-entry level. `HasFourZoneRgb`/`SupportsFanCurves` were kept `true`, since #130 independently confirms basic keyboard color control and a successful Fan Calibration Wizard run — real, if partial, positive evidence rather than an inherited assumption.

**Tests.** Two new regression tests pin the corrected values down explicitly and explain why in their failure messages: `GetCapabilities_8E10_MaxFanLevel_MatchesFieldConfirmedRealCeiling_NotNominal` and `GetCapabilities_8E10_DoesNotClaimUnconfirmedMuxOrGpuPowerBoost`.

**Process note, not a code note:** this is the second near-miss this cycle from the same root cause (adding a database entry from one issue's evidence without first searching for *other* issues about the same ProductId — the first was the `88D2` duplicate, caught by a test failure; this one would not have failed any test). Worth treating "grep every other open/closed issue mentioning this exact ProductId" as a mandatory step before adding or editing any board entry, not an optional nicety — the codebase and issue history have both grown large enough that a single report is no longer reliably the complete picture for a given board.

---

### GitHub #128 Follow-Up: `88EC` Performance Mode Switches Did Nothing At All

**Report:** Follow-up diagnostics bundle on [#128](https://github.com/theantipopau/omencore/issues/128) (HP Victus 16-e0xxx, board `88EC`, OmenCore v4.2.0), submitted in response to the request left on that issue during this cycle's backlog sweep. Two full session logs plus the "priority model validation cards"/RC-matrix diagnostics this project's own field-validation tooling produces.

**Good news first, confirmed by the fresh data:** the WMI fan-write path genuinely works on this board. `core-control-readiness.txt` and the Sept 6 session log show a real thermal-protection event firing correctly — CPU sustained 91.1°C for 6s, `⚠️ THERMAL WARNING: 91°C sustained for 6s - boosting fan speed` fired, fan levels were set to 48 (88% duty), read back as `Valid` state (4600/4800 RPM), and the countdown-extension keepalive held the setting. `ec-state.txt`'s raw EC register dump shows implausible temperature values at the classic legacy offsets (e.g. `EC[0x63]=119°C`, `EC[0x69]=116°C` — not real operating temperatures), confirming this board's legacy EC register map is genuinely unconfirmed/unsafe and that `SupportsFanControlEc=false` is the correct call, not overcaution.

**The real, confirmed bug:** the Sept 3 session log contains `⚠️ Performance mode 'Balanced': nothing was applied (Direct EC writes disabled for model 'HP Victus 16-e0xxx')`. Traced in `PerformanceModeService.Apply`: when Direct EC power-limit writes are unavailable (correctly, for this board) and `AllowDecoupledWmiThermalPolicyFallback` is `false`, there is no other path left to apply *anything* for a mode switch — no EC limits, no WMI thermal-policy fallback, and `LinkFanToPerformanceMode` defaults `false` too, so `appliedEffects` ends up empty and the mode switch is a genuine no-op. This exact failure shape was already fixed for four sibling Victus/OMEN boards this project has touched before (`8DCD`, `8C30`, `878C`, `8600`) by setting `AllowDecoupledWmiThermalPolicyFallback = true` — `88EC` simply never received the same fix, despite being the same class of board (WMI-only, no confirmed EC).

**Fix.** Added `AllowDecoupledWmiThermalPolicyFallback = true` to `88EC`'s `ModelCapabilities` entry — the exact same one-flag fix already proven on four other boards. This routes Performance-mode switches through the WMI thermal-policy keepalive path (`SetFanPerformanceModeSerialized`) instead of a dead end. No new write path — this only enables an already-shipped, already-tested fallback mechanism for one more board that needed it.

**Tests.** New `GetCapabilities_88EC_AllowsDecoupledWmiThermalPolicyFallback` (`ModelCapabilityDatabaseTests.cs`).

**A second, cross-board pattern surfaced, not actioned this pass:** the Sept 3 log shows `[WmiBiosMonitor] CPU thermal authority switched` between `ACPI Thermal Zone` and `LHM Fallback` **20 times** in a single session, almost always for a ~1.0-1.5°C difference between the two sources (e.g. `82.1C vs 83.6C`) — right at `WmiBiosMonitor`'s `Math.Abs(fallbackCpuTemp - _cachedCpuTemp) >= 1.0` reconsideration threshold. This is the same magnitude and pattern already noted as a one-off observation on a *different* board (`8C2F`/#184, "switched sources 20+ times across the session's four log files... worth another look if a pattern shows up across more reports rather than acting on a single bundle now"). It now has: two independent boards, matching frequency (~20/session), and a plausible concrete cause (a 1.0°C threshold is within normal sensor noise for two independently-sourced readings, especially two different physical sensors/refresh cadences being compared against each other). **Not fixed here** — this selection logic decides which temperature reading drives fan-safety behavior fleet-wide, and loosening/adding hysteresis to it needs careful thought about what it could mask (a real, fast-rising temperature divergence) before touching it. Recorded as a real candidate for a dedicated pass, not guessed at blind.

---

### Built-In RGB Scene Scheduling Silently Overrode Manually-Configured Lighting

**Report:** Discord ("snowfall hateall"), 2026-09-06 — OMEN MAX 16-ak003nr, board `8D87`/`88F7`, OmenCore v4.2.0. "the rgb light bar randomly turned orange without any command from the user. It is configured to be off, yet randomly switched to orange without warning." Attached `F:\Downloads\logs.txt` (39 lines, a short window around the event) — the log itself names the exact mechanism, not a vague symptom: `[INFO] Scheduled time triggered scene 'Night Mode'` at `22:00:26`, immediately followed by `[WmiBiosBackend] Setting colors: Z0=#331100, Z1=#331100, Z2=#331100, Z3=#331100` and `✓ Keyboard color table set`. `#331100` is a dark amber/orange — exactly matching "turned orange."

**Traced, not guessed.** `RgbSceneService.InitializeBuiltInScenes()` (line ~128) seeds two built-in scenes with a live schedule baked directly into their object initializers: `"night"` (`PrimaryColor = "#331100"`, `Brightness = 30`, `ScheduledTime = "22:00"`, every day) and `"work"` (`PrimaryColor = "#FFFFFF"`, `Brightness = 80`, `ScheduledTime = "09:00"`, Mon-Fri). `CheckScheduledScenes()` (line ~589), run every minute by a `Timer` started in the service constructor, applies **any** scene whose `ScheduledTime` matches the current `HH:mm`, gated only by `IsSchedulingEnabled` (line 58) — a property that **defaults to `true` and has no other setter anywhere in the codebase** (confirmed via grep: the only two references to `IsSchedulingEnabled` in the entire solution are its own declaration and the `if (!IsSchedulingEnabled) return;` guard inside `CheckScheduledScenes` itself).

**No UI surface exists for any part of this feature** — confirmed via grep across `LightingView.xaml`/`LightingViewModel.cs`: no toggle for `IsSchedulingEnabled`, no way to see a scene has a `ScheduledTime`, no editor to set, change, or clear one on any scene (`CreateSceneFromCurrent`, the only in-app scene-creation path, never touches `ScheduledTime`/`ScheduledDays`). This means every installed copy of OmenCore, with zero configuration from the user, silently changes keyboard lighting to a dim amber "Night Mode" every night at 10PM and white "Work" lighting every weekday morning at 9AM — overriding whatever the user manually configured, including an explicit "Off," with no way to see it coming or turn it off short of editing `%APPDATA%\OmenCore\rgb_scenes.json` by hand (a file that doesn't even exist until a user has explicitly customized a scene at least once, since `SaveScenes()` is the only writer and it's never called automatically).

**Same failure shape as `PowerAutomationService`'s startup-overwrite bug fixed earlier this cycle** — a background automation mechanism silently overriding explicit user state, this time with the added problem that there's no way to discover or disable it at all, not even after the fact.

**Fix.** Removed `ScheduledTime`/`ScheduledDays` from both built-in scene definitions. Both scenes are otherwise unchanged and remain fully selectable from the scene list — a user who wants a warm evening color can still click "Night Mode" manually, exactly as before. The scheduling engine itself (`CheckScheduledScenes`, the `_scheduleTimer`, `IsSchedulingEnabled`, `GetScheduledScenes()`) is deliberately left in place, not removed — it's not wrong on its own terms, it's just that no scene should carry a live schedule until a real opt-in UI exists to show and control it, which is a separate, larger feature (a settings toggle plus a per-scene schedule editor) not attempted here.

**Considered and rejected:** defaulting `IsSchedulingEnabled` to `false` instead of/in addition to fixing the two built-in scenes. Rejected as unnecessary for this fix specifically (removing the only two schedules that exist anywhere in the codebase already eliminates 100% of the live triggers), though flagged below as worth doing anyway for defense-in-depth against a hypothetical future scene arriving with a schedule (e.g., via a hand-edited `rgb_scenes.json`, or a future import feature) before the real opt-in UI ships.

**Tests.** New `BuiltInScenes_NeverShipWithASilentDefaultSchedule` in `RgbSceneServiceTests.cs`, asserting no scene in `service.Scenes` carries a `ScheduledTime` — a genuine regression guard, since `RgbSceneServiceTests.cs` already had a real, working `CreateService` test harness from prior coverage. Full suite: 1408/1408 (up from 1407).

**Not a field-validation item.** This removes an unrequested default write path; it adds no new one, and doesn't touch how a scene is actually applied to hardware once selected.

**Follow-up not attempted here:** default `IsSchedulingEnabled` to `false` for defense-in-depth (see above), and eventually build the real opt-in UI (a Settings/Lighting toggle plus a per-scene schedule editor) if scene scheduling is a feature worth keeping rather than quietly dropping.
...
Added `8D26` as its own entry in both `ModelCapabilityDatabase.cs` (alongside the existing `8D24`/`8E35` siblings in the same 16-ap0xxx family — same V1 WMI fan control, `MaxFanLevel=55`, 2 fan zones, `SupportsFanControlEc=false`/`SupportsUndervolt=false` since this is AMD Ryzen AI, no Intel MSR undervolt path) and `KeyboardModelDatabase.cs` (same `ColorTable2020` V1 WMI keyboard interface). Every flag mirrors what `8D24`/`8E35` already carry — this is a pure identity fix, not a new capability claim, matching the same pattern used for `8BA9`/`8603` earlier this cycle. `UserVerified` stays `false` since no `field-validation-script.txt` pass was attached to the report.

**Tests:** `GetCapabilities_8D26_Ap0xxxAmd_UsesExactV1WmiProfile` (`ModelCapabilityDatabaseTests.cs`) and `GetConfig_ReturnsConfig_For_ProductId_8D26` (`KeyboardModelDatabaseTests.cs`), mirroring the existing `8E35` spot-check tests exactly.

---

### Windows: OMEN Key WMI Watcher Registered Successfully Yet Never Received a Single Event on Some Boards

**Report:** [#187](https://github.com/theantipopau/omencore/issues/187) — HP Victus 16-e0054nl, board `88EE`, Ryzen 7 5800H + RTX 3060, OmenCore v4.2.0. Exceptionally well-isolated field report. Log showed a clean, error-free watcher startup: `✓ WMI event watcher started: root\wmi - SELECT * FROM hpqBEvnt WHERE eventId = 29 AND eventData = 8613`, `✓ OMEN key interception started` — then **nothing** across two separate ~15-30 second test sessions of physically pressing the OMEN key. Already ruled out: leftover PowerShell WMI subscriptions, `strictOmenKeyMode`, EC backend disabled, non-elevated process. Reporter's own independent proof the hardware/firmware side works: `Register-WmiEvent -Class "hpqBEvnt" -Namespace "root\wmi" -Action {...}` — **no WHERE clause at all** — received the identical key press instantly and reliably every time, printing `EventID=29`/`EventData=8613`/`InstanceName=ACPI\PNP0C14\0_0` exactly as expected once the captured event's properties were inspected.

**Confirmed via code, not assumed:** `OnWmiEventArrived` (the `EventArrived` handler `StartWmiEventWatcher` attaches) independently re-extracts `eventId`/`eventData` from the raw event and fails closed (returns, no side effect, Debug-only log) unless both match exactly — this is a complete, correct, already-shipped client-side filter, entirely independent of whatever WQL WHERE clause the subscription itself used. The reporter's log showing **zero** further lines — not even the unconditional `_logging.Debug($"WMI event received: class={className}...")` line that fires the instant `OnWmiEventArrived` is invoked, before any eventId/eventData check — proves the handler was never called at all. This rules out a client-side filtering bug (the reporter's own leading theory) and isolates the fault to the subscription itself: the server-side WQL WHERE-clause match against `eventId`/`eventData` was silently never evaluating true on this board's `hpqBEvnt` schema, despite the identical values reading back correctly once an event instance is actually captured (exactly what the reporter's unfiltered manual test did, and exactly what the filtered subscription's WHERE clause presumably could not match server-side).

**Root cause, best-evidenced explanation (not provable without the exact schema on this board's BIOS):** `hpqBEvnt` is a WMI class exposed via the ACPI-WMI mapper for a vendor-specific ACPI method output, and different HP BIOS/board revisions are known to vary in how they type-map that raw ACPI buffer into WMI CIM properties. A WQL numeric-literal `WHERE` comparison (`eventData = 8613`) requires the server-side property's CIM type to match what the literal parses as; if this board's `hpqBEvnt.eventData`/`eventId` don't match that expectation, the WHERE clause can silently and permanently fail to match without erroring the subscription itself — exactly the observed symptom (`.Start()` succeeds, `EventArrived` never fires). The original code's own comment ("DO NOT use broad queries like `SELECT * FROM hpqBEvnt`... causes focus-stealing behavior") pre-dates the current, fully-defensive client-side re-validation in `OnWmiEventArrived` and no longer reflects what that handler actually does — traced its full body end to end and confirmed **no user-visible action of any kind occurs before the `eventId==29`/`eventData==8613` check passes**, so a broader subscription cannot reintroduce the focus-stealing the old comment warned about.

**Fix.** `StartWmiEventWatcher` now subscribes with `SELECT * FROM hpqBEvnt` — no WHERE clause, textually matching the scope of the reporter's own proven-reliable manual test exactly — collapsing what were three separate query-string variants (default / fnP-only / fnP+legacy-combined) down to one, since all three cases were already fully and correctly handled downstream by `OnWmiEventArrived`'s existing `isFirmwareFnPProfileCycle`/`IsHookActive`/exact-match logic regardless of what the server-side query fetched. Also switched from the simple `ManagementEventWatcher(ns, queryStr)` constructor to an explicit `ManagementScope` with `ConnectionOptions { EnablePrivileges = true }` — a well-known WMI eventing gotcha for classes backed by a kernel/ACPI provider rather than a plain CIM repository class, added defensively alongside the query fix since it costs nothing for a query that might not need it, in case the real or an additional cause is privilege-related rather than purely the query-type mismatch.

**Verification.** Full solution build clean; full test suite unaffected (no existing test constructs a real `ManagementEventWatcher` against live WMI — `OmenKeyService` has no dedicated test file, consistent with other hardware/OS-eventing-coupled classes in this codebase like `LinuxEcController`/`LibreHardwareMonitorImpl`). **Not verified against real hardware** — this environment cannot reproduce a physical OMEN key press or inspect this board's actual `hpqBEvnt` CIM schema. Framed as implemented-pending-confirmation: the downside if the type-mismatch theory is wrong is "still doesn't fire" (no worse than before), never a new false-positive or focus-stealing regression, since the unchanged client-side filter is what actually gates every user-visible action.
...
### GitHub #126 — Linux `LinuxEcController.HasEcAccess` Is a Denylist, Not an Allowlist — Same Reason-Text Bug, Plus a Bigger Open Question

**Report:** [#126](https://github.com/theantipopau/omencore/issues/126) — HP Victus 16-r0xxx, board `8BC2`, Ubuntu 26.04/kernel 7.0.0. Same symptom shape as [#127](https://github.com/theantipopau/omencore/issues/127) above and fixed by the same reason-text change: diagnose output shows `ec_sys`/`ec_io` diagnostics all positive and `Capability: full-control`, but the stated reason named "hp-wmi hwmon pwm/fan targets" while `thermal_profile`/`fan1_target`/`fan2_target`/`fan1_output`/`fan2_output` are all reported missing — the exact `hasEcAccess`-was-the-real-reason-but-hwmon-got-named-instead mismatch the `LinuxCapabilityClassifier` fix above corrects.

**A bigger question surfaced while tracing this, not actioned:** `LinuxEcController.HasEcAccess = File.Exists(EC_PATH) && !IsUnsafeEcModel` — `IsUnsafeEcModel` checks board ID / model name against `UnsafeEcBoardIds`/`UnsafeEcModelPatterns`, two explicit **denylists** (currently: `8c58`, `8d24`, `8d26`, `8e35`, `8e41`, and a handful of 2025 `16-ah0`/`16-ap0`/`17-ah0`/`transcend 14` name patterns). This means **any board not explicitly denylisted is assumed EC-write-safe by default** — the inverse of the discipline `ModelCapabilityDatabase.cs`'s Windows-side conservative-fallback fix (this cycle's `DefaultCapabilities`/`GetCapabilitiesByFamily` fix, and the class-level-defaults audit flagged below) already applies: assume unsupported/unsafe until a specific board confirms otherwise, not the other way around. `8BC2` isn't on either denylist, so `HasEcAccess` is `true` for it purely by not having been flagged bad yet — not because anyone has confirmed OmenCore's EC register offsets (`REG_CPU_TEMP = 0x57`, `REG_GPU_TEMP = 0xB7`, and whatever offsets the fan-write path uses) are correct for this exact board.

**Not actioned, and deliberately not treated as a quick fix.** Whether this denylist-by-default design was a considered, deliberate choice in an earlier session (e.g., because the specific CPU/GPU temperature register offsets genuinely are stable across the whole legacy HP EC family, and only *fan-write* offsets vary in ways the denylist already tracks per known-bad board) or a real gap is not something this pass can determine with confidence — flipping it to an allowlist is a significant EC *write*-safety architecture change squarely inside the evidence gate's fan/EC field-validation bar, not a display-honesty fix, and risks breaking real fan control on every currently-working non-denylisted board if done carelessly. Flagged here for the owner's judgment call, not decided or changed.

**Reply posted** confirming the reason-text fix applies here too, and asking whether a concrete write test (`sudo omencore-cli fan --profile max`, checked against actual RPM change) succeeds on this hardware — that result would settle whether this board's `FullControl` classification is accurate or whether the denylist-vs-allowlist question above is live for real boards today, not just hypothetical.
...
### GitHub #189 — Exceptionally Well-Researched Feature Request: Opt-In Software Fan Curve Controller for Boards Where Firmware Auto Under-Cools

**Report:** [#189](https://github.com/theantipopau/omencore/issues/189) — HP OMEN MAX 16-ak0xxx, board `8D87`, BIOS `F.07`, Linux (Arch/Omarchy, kernel 7.1.9). One of the most thorough field reports this project has received: the reporter decompiled HP OMEN Gaming Hub's Windows assemblies (ILSpyCmd) to extract its actual fan-control algorithm, then built and validated a standalone Linux prototype reproducing it against real hardware.

**The problem, confirmed by direct measurement, is not that OmenCore/Linux can't command fans.** With `platform_profile=performance` and firmware left in `pwm1_enable=2` (Auto), sustained CPU load stabilizes at a firmware-imposed ceiling of ~3,400/3,600 RPM while CPU temperature climbs to 92-99°C — repeatable across multiple capture runs, all landing on the exact same RPM ceiling regardless of temperature. Manual control on the same hardware reaches ~6,000 RPM, so the fans and the OS-to-firmware fan-write path both work; firmware's own Auto algorithm simply doesn't push hard enough under this profile.

**Root cause, identified via decompilation, not guessed:** Gaming Hub's `FanHandler.GetFanSetScheme` runs a full userspace EWMA (exponentially-weighted moving average) controller for Auto whenever the system isn't idle — sampling CPU, GPU, and IR/chassis temperature once a second, applying independent asymmetric rise/fall curves per sensor, taking the maximum of the three, and mapping the winning level through a board-specific 128-byte fan table (HP WMI command `0x2f`) before writing discrete levels via `0x2e`. This entire userspace policy layer has no equivalent anywhere in OmenCore (or, per the report, in Linux's own `hp-wmi` kernel driver) — firmware's own `pwm1_enable=2` "Auto" is a much simpler, more conservative built-in policy, and Gaming Hub's real "Auto" behavior on Windows is actually this external software loop layered on top of it.

**Validated on real hardware, not just theorized:** the reporter's standalone Python prototype (using only the standard `hp-wmi` hwmon ABI, no OmenCore code) reproduced the factory Performance curve table exactly (extracted via a live eBPF capture of the WMI `0x2f` response, SHA-256-verified) and, under the same sustained load, held CPU at 85.5°C instead of 92-99°C by commanding fans up to ~4,600/4,800 RPM. Multiple controlled test runs (CPU-only, GPU-only, combined CPU+GPU, with a 92°C raw-temperature emergency override) all completed without needing the emergency path except in one deliberately extreme synthetic combined-load test, and every run correctly restored firmware Auto on exit.

**Proposed feature (not attempted here):** an opt-in, safety-gated automatic software fan-curve controller — sample sensors once/second, apply per-sensor asymmetric curves, select the max, map through the board's own `0x2f` table, write only changed levels, and restore Auto on any exit/profile-change/suspend/crash path. The report explicitly frames the needed safety invariants: strict curve validation, per-board allowlisting (not board-ID-alone), a raw critical-temperature override independent of the smoothed control loop, sensor-loss fail-open handling, rate limiting, and full telemetry. Linux kernel 7.1's `hp-wmi` driver already shares this same fan-table implementation across at least 17 other board IDs (`8902, 8A44, 8A4D, 8BAB, 8BBE, 8BC2, 8BCA, 8BCD, 8BD4, 8BD5, 8C76, 8C77, 8C78, 8C99, 8C9C, 8D41, 8D87`) — meaning this isn't necessarily an `8D87`-only gap, though the report is careful to note that shared driver code doesn't imply identical defects or fan-table mappings on every one of those boards.

**Not attempted this pass — this is a genuinely new, substantial feature, not a bug fix.** It needs its own dedicated design pass (a new controller service, curve-file validation, a model-allowlist mechanism, a real recovery/watchdog story for crash/`SIGKILL` cases) and real hardware to validate against beyond the reporter's own board — exactly the shape of change the evidence gate's field-validation bar exists for, amplified here because it's proposing an entirely new automatic write-loop rather than fixing an existing one. Replied on the issue thanking the reporter for exceptional research and confirming this is recorded for a dedicated future pass rather than rushed into this cycle.

**Follow-up (2026-09-06):** reporter (`mbilykov`) confirmed OmenCore is free to use the reverse-engineered data, and confirmed their own goal matches exactly what was suggested — continuing the standalone prototype toward a real Linux daemon, with more data to follow once it's ready. Worth checking this issue again before starting the design pass rather than assuming the prototype is still where this entry describes it.

### Community Resource — `bobshmo/Victus-SW_lim-delete` (Discord, 2026-09-06): GPU Platform-Power-Cap Unlock for HP Victus RTX 5060

Discord reports (`bobshmo`) of a ~90-125W GPU platform power cap on HP Victus laptops with RTX 5060 GPUs (stock OMEN VBIOS caps at 90W per one report), and a self-described **"vibecoded"** (the reporter's own word — AI-generated, not independently reviewed) unlock published at [`bobshmo/Victus-SW_lim-delete`](https://github.com/bobshmo/Victus-SW_lim-delete). The reporter also separately mentions needing to cap voltage at 1000mV via a third-party tool "to prevent crashes on the recommended HP driver" once the power limit is lifted.

**Not actioned, and not recommended without independent review.** This is a fundamentally different risk class from every other community tool referenced in this document (`omen-acpi`, OmenMon-Reborn, OmenXHub) — those patch ACPI tables or reimplement already-understood, read-mostly control surfaces; this one is described by its own author as AI-generated and directly modifies GPU power/voltage delivery limits, with the author's own follow-up noting it can cause crashes serious enough to need a separate manual voltage cap to work around. Unreviewed, undocumented voltage/power-limit modifications carry real hardware-damage risk (not just a crash-and-recover risk) if the underlying register writes are wrong. Recorded here purely as an awareness note, not a recommendation — should not be referenced to users as a fix, and definitely not a candidate for porting logic from, without a qualified independent review of exactly what it writes and why.
